...
int textFunctions::numval(string q) {
  ...
  //      << " int_limit = " << std::numeric_limits<int>::max() 
  //      << " errno = " << errno << endl;
...
unsigned int textFunctions::unumval(string q) {
  ...
  if (endptr == str)
    errno = EINVAL;
  if ((qv < 0) || (qv > std::numeric_limits<unsigned int>::max()))
    errno = ERANGE;
...
bool textFunctions::endswith (string const &fullString, string const &ending) {
  unsigned int lastMatchPos = fullString.rfind(ending);
  bool isEnding = lastMatchPos != std::string::npos;

  // If the string was found, make sure that any characters that follow it 
  // are the ones we're trying to ignore
  for( int i = lastMatchPos + ending.length(); 
       (i < fullString.length()) && isEnding; 
       i++) {
    if( !isWS(fullString[i]) )
      {
	isEnding = false;
      }
  }  
...
void textFunctions::rchars(string& source,char target,char substitute) {
  for (int i=0;i<source.length();i++) {
    if (source[i] == target) source[i]=substitute;
  }
}
