...
file = File.new("/home/emily/Desktop/ALFRED/alf_mcl/etc/mclhost.#{ENV['USER']}", "w")
	...
	if com.start_with? "monitor"
		...
		if com.start_with? "failed"
			...
			if initialized == false
				...
				localhost.cmd(com) { |c| response = c}
				com = %q[configure(pause_time,wiztest,wiztest)]
				localhost.cmd(com) { |c| response = c}
				com = %q[declareObservableSelf(pause_time,pauselen,1)]
				localhost.cmd(com) { |c| response = c}
				com = %q[setObsPropSelf(pause_time,pauselen,prop_sclass,sc_counter)]
				localhost.cmd(com) { |c| response = c}
				...
				#Initialize Num_Violations
				com = %q[initialize(violations)]
				localhost.cmd(com) { |c| response = c}
				com = %q[configure(violations,alfviolations,alfviolations)]
				localhost.cmd(com) { |c| response = c}
				com = %q[declareObservableSelf(violations,num_violations,1)]
				localhost.cmd(com) { |c| response = c}
				com = %q[setObsPropSelf(violations,num_violations,prop_sclass,sc_counter)]
				localhost.cmd(com) { |c| response = c}
				com = %q[declareEG(violations,1)]
				localhost.cmd(com) { |c| response = c}
				com = %q[declareSelfExp(violations,1,ec_stayunder,num_violations,]+ mcl_violations.to_s + ')'
				localhost.cmd(com) { |c| response = c}
				initialized = true
			end
