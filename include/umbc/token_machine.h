...
namespace umbc {
  ...
  public:
    ...
    virtual string nextToken(); //<! gets the next token in the string
  ...
  public:
    ...
    virtual string getParam(int X);
    virtual string getNextParam() { return nextToken(); };
    bool   hasMoreParams() { return moreTokens(); };
    bool   isLegit(string paramVal) { return paramVal != ""; };
    void   paramListRest(vector<string>& dest);
    void   paramListAll(vector<string>& dest);
